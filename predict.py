# predict.py
# Runs inference on two example images and saves them with predicted bounding boxes drawn on.
...
def predict_boxes(model, image: np.ndarray, conf_threshold: float = 0.5) -> list:
    """
    Runs the model on a single normalised image and returns predicted boxes
    above the confidence threshold.

    Returns a list of dicts with keys: class, xmin, ymin, xmax, ymax, confidence
    """
    inp = np.expand_dims(image, axis=0)
    pred = model.predict(inp, verbose=0)[0]  # (GRID_S, GRID_S, 5 + NUM_CLASSES)
    ...
    for row in range(GRID_S):
        for col in range(GRID_S):
            ...
            # Convert from cell-relative to absolute pixel coordinates
...
def plot_predictions(model, images: list, n: int = 2):
    """
    Saves a figure with n example images and the predicted bounding boxes drawn on them.
    """
    ...
    for ax, img in zip(axes, images[:n]):
        # img is normalised float — convert back to uint8 for display
